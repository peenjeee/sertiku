...
import { Space, generate, fromMnemonic, toMnemonic, OwnedSpace, SharedSpace, fromDelegation, createRecovery, SESSION_LIFETIME, } from '../src/space.js';
...
export const testSpace = Test.withContext({
    ...
    'should default to public accessType when not specified': async (assert, { client }) => {
        const signer = await Signer.generate();
        const name = `space-${Date.now()}`;
        const space = new Space({
            id: signer.did(),
    ...
    'should get usage': async (assert, { client, grantAccess, mail }) => {
        ...
        if (!found)
            return assert.fail('space not found');
        const usage = Result.unwrap(await found.usage.get());
        assert.equal(usage, BigInt(size));
    },
    'should generate space with generate function': async (assert, { client }) => {
    ...
    'should convert space to mnemonic and recover': async (assert, { client }) => {
    ...
    'should create authorization and fromDelegation': async (assert, { client }) => {
        const space = await generate({
            name: 'test-space',
            access: {
                type: 'private',
    ...
    'should test SharedSpace withName method for private space': async (assert, { client }) => {
        const space = await generate({
            name: 'shared-test',
            access: {
                type: 'private',
    ...
    'should test SharedSpace withName method for public space': async (assert, { client }) => {
        const space = await generate({
            name: 'shared-test',
            access: { type: 'public' },
            agent: client.agent,
    ...
    'should test SharedSpace withName method and default to public accessType': async (assert, { client }) => {
    ...
    'should test encryptionProvider and encryptionAlgorithm getters for all cases': async (assert, { client }) => {
        ...
        if (privateSpace.access.type === 'private') {
            ...
            assert.equal(privateSpace.access.encryption.algorithm, 'RSA_DECRYPT_OAEP_3072_SHA256');
        }
        // Test public space
