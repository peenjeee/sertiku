...
export const BlobClient = Test.withContext({
    'should store a blob': async (assert, { connection, provisionsStorage, registry }) => {
    ...
    'should replicate a blob': async (assert, { connection, provisionsStorage }) => {
        const bytes = await randomBytes(128);
        ...
        const replicas = 1;
        const result = await alice.capability.blob.replicate({ digest, size: bytes.length }, site, replicas);
