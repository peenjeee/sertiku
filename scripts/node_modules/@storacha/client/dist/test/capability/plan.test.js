...
export const PlanClient = Test.withContext({
    get: {
        'should get a plan': async (assert, { client, plansStorage, grantAccess, mail }) => {
            ...
            Result.try(await plansStorage.initialize(account.did(), 'stripe:123xyz', exampleProduct));
    ...
    set: {
        'should set a plan': async (assert, { client, plansStorage, grantAccess, mail }) => {
            const account = await initializeAccount(client, mail, grantAccess);
            ...
            Result.try(await plansStorage.initialize(account.did(), 'stripe:123xyz', initialProduct));
            ...
            assert.equal((await client.capability.plan.get(account.did(), { nonce: '2' }))
                .product, updatedProduct);
            await assert.rejects(client.capability.plan.set('did:mailto:example.com:notauser', initialProduct, { nonce: '3' }));
        },
    },
    createAdminSession: {
        'should create an admin session': async (assert, { client, plansStorage, grantAccess, mail }) => {
            const account = await initializeAccount(client, mail, grantAccess);
            await assert.rejects(client.capability.plan.createAdminSession(account.did(), 'https://example.com/return-url'));
            const initialProduct = 'did:web:example.com';
            Result.try(await plansStorage.initialize(account.did(), 'stripe:123xyz', initialProduct));
            const session = await client.capability.plan.createAdminSession(account.did(), 'https://example.com/return-url', { nonce: '2' });
            assert.ok(session.url);
            await assert.rejects(client.capability.plan.createAdminSession('did:mailto:example.com:notauser', 'https://example.com/return-url', { nonce: '3' }));
        },
    },
    createCheckoutSession: {
        'should create a checkout session': async (assert, { client, plansStorage, grantAccess, mail }) => {
            const account = await initializeAccount(client, mail, grantAccess);
            const session = await client.capability.plan.createCheckoutSession(account.did(), {
            ...
            await assert.rejects(client.capability.plan.createCheckoutSession(errorUser.did(), {
                successURL: 'https://example.com/return-url',
                cancelURL: 'https://example.com/cancel-url',
                planID: 'did:web:example.com',
                freeTrial: false,
