...
export const SpaceClient = Test.withContext({
    info: {
        'should retrieve space info': async (assert, { connection, provisionsStorage }) => {
    ...
    record: {
        'should record egress if the capability is derived from *': async (assert, { id: w3, connection, provisionsStorage }) => {
            ...
            const freewayService = new Client(await AgentData.create({
            ...
            assert.ok(freewayDelegations.some((d) => d.issuer.did() === alice.did() &&
                d.audience.did() === freewayService.did() &&
                d.capabilities.some((c) => c.can === Space.egressRecord.can && c.with === space.did())));
            ...
            try {
                ...
                const egressRecord = await freewayService.capability.space.egressRecord(egressData, {
                    proofs: await freewayService.proofs(),
                });
                assert.ok(egressRecord, 'egressRecord should be returned');
                assert.equal(egressRecord.space, space.did(), 'space should be the same');
                assert.equal(egressRecord.resource.toString(), resource.toString(), 'resource should be the same');
                assert.equal(egressRecord.bytes, car.size, 'bytes should be the same');
                assert.equal(new Date(egressRecord.servedAt).getTime(), Math.floor(new Date(egressData.servedAt).getTime() / 1000), 'servedAt should be the same');
        ...
        'should record egress if the capability is derived from space/*': async (assert, { id: w3, connection, provisionsStorage }) => {
            const expiration = Date.now() + 1000 * 60 * 60 * 24; // 1 day from now
            // 1. Setup test space and allow Alice Agent to access it
            const alice = new Client(await AgentData.create(), {
                // @ts-ignore
            ...
            assert.ok(freewayDelegations.some((d) => d.issuer.did() === alice.did() &&
                d.audience.did() === freewayService.did() &&
                d.capabilities.some((c) => c.can === Space.top.can && c.with === space.did())));
            // 5. Create a random resource to record egress
            const car = await randomCAR(128);
            const resource = car.cid;
            assert.ok(resource);
        ...
        'should record egress if the capability is derived from space/content/serve/*': async (assert, { id: w3, connection, provisionsStorage }) => {
            const expiration = Date.now() + 1000 * 60 * 60 * 24; // 1 day from now
            // 1. Setup test space and allow Alice Agent to access it
            const alice = new Client(await AgentData.create(), {
                // @ts-ignore
            ...
            assert.ok(freewayDelegations.some((d) => d.issuer.did() === alice.did() &&
                d.audience.did() === freewayService.did() &&
                d.capabilities.some((c) => c.can === Space.contentServe.can && c.with === space.did())));
            // 5. Create a random resource to record egress
            const car = await randomCAR(128);
            const resource = car.cid;
            assert.ok(resource);
        ...
        'should record egress if the capability space/content/serve/egress/record is delegated': async (assert, { id: w3, connection, provisionsStorage }) => {
            const expiration = Date.now() + 1000 * 60 * 60 * 24; // 1 day from now
            // 1. Setup test space and allow Alice Agent to access it
            const alice = new Client(await AgentData.create(), {
                // @ts-ignore
        ...
        'should fail to record egress if the capability was not delegated': async (assert, { id: w3, connection, provisionsStorage }) => {
            const expiration = Date.now() + 1000 * 60 * 60 * 24; // 1 day from now
            // 1. Setup test space and allow Alice Agent to access it
            const alice = new Client(await AgentData.create(), {
                // @ts-ignore
            ...
            try {
                ...
                assert.equal(
                // @ts-ignore
                error.message, `failed ${Space.egressRecord.can} invocation`, 'error message should be the same');
            }
        },
    },
});
