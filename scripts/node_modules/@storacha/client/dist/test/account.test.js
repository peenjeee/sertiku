...
export const testAccount = Test.withContext({
    ...
    'login idempotence': async (assert, { client, mail, grantAccess }) => {
        ...
        assert.deepEqual(Object.keys(client.accounts()), [alice.did()], 'no accounts have been saved');
        const retry = await client.login(email);
        assert.deepEqual(alice.toJSON(), retry.toJSON(), 'same account view is returned');
        const loginResult = await Account.login(client, email);
        assert.deepEqual(alice.toJSON(), loginResult.ok?.toJSON(), 'same account is returned with low level API');
    },
    ...
    'create account and provision space': async (assert, { client, mail, grantAccess }) => {
        ...
        assert.deepEqual(space.signer.encode(), signer.encode(), 'arrived to same signer');
        const email = 'alice@web.mail';
        const login = Account.login(client, email);
        const message = await mail.take();
        assert.deepEqual(message.to, email);
    ...
    'check and set account plan': async (assert, { client, mail, grantAccess, plansStorage }) => {
        const login = Account.login(client, 'alice@web.mail');
        await grantAccess(await mail.take());
        const account = Result.try(await login);
        const { error } = await account.plan.get();
        assert.ok(error);
        Result.unwrap(await plansStorage.initialize(account.did(), 'stripe:123xyz', 'did:web:free.storacha.network'));
    ...
    'create plan admin session': async (assert, { client, mail, grantAccess, plansStorage }) => {
        ...
        const account = Result.try(await login);
        await account.save();
        Result.try(await plansStorage.initialize(account.did(), 'stripe:123xyz', 'did:web:example.com'));
        const { ok } = await account.plan.createAdminSession(accountDID, 'https://example.com');
        assert.ok(ok);
        assert.ok(ok?.url);
    },
    'check account subscriptions': async (assert, { client, mail, grantAccess }) => {
        const space = await client.createSpace('test', {
            skipGatewayAuthorization: true,
        });
        const email = 'alice@web.mail';
    ...
    waitForPaymentPlan: {
        'should wait for a payment plan to be selected': async (assert, { client, mail, grantAccess }) => {
            const email = 'alice@web.mail';
            const login = Account.login(client, email);
            await grantAccess(await mail.take());
            const account = Result.try(await login);
        ...
        'should throw an error if there is an issue retrieving the payment plan': async (assert, { client, mail, grantAccess }) => {
            const email = 'alice@web.mail';
            const login = Account.login(client, email);
            await grantAccess(await mail.take());
            const account = Result.try(await login);
            ...
            await assert.rejects(account.plan.wait({ interval: 100, timeout: 1000 }), {
                message: 'Error retrieving payment plan: "Some error"',
            });
        },
        'should throw a timeout error if the payment plan selection takes too long': async (assert, { client, mail, grantAccess }) => {
            const email = 'alice@web.mail';
            const login = Account.login(client, email);
            await grantAccess(await mail.take());
            const account = Result.try(await login);
            ...
            await assert.rejects(account.plan.wait({ interval: 100, timeout: 500 }), {
                message: 'Timeout: Payment plan selection took too long.',
            });
        },
        'should throw an error when the abort signal is aborted': async (assert, { client, mail, grantAccess }) => {
