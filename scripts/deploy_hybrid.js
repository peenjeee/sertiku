/**
 * Deploy CertificateRegistryHybrid Smart Contract to Polygon Amoy Testnet
 * Gas-optimized version: ~150,000 gas vs ~721,000 gas
 * 
 * Usage: node scripts/deploy_hybrid.js
 */
...
function compileContract() {
    console.log('Compiling CertificateRegistryHybrid.sol...');

    const contractPath = path.join(__dirname, '..', 'contracts', 'CertificateRegistryHybrid.sol');
    const source = fs.readFileSync(contractPath, 'utf8');

    const input = {
        language: 'Solidity',
        sources: {
            'CertificateRegistryHybrid.sol': {
                content: source
            }
        },
        settings: {
    ...
    const contract = output.contracts['CertificateRegistryHybrid.sol']['CertificateRegistryHybrid'];
...
async function deployContract() {
    console.log('==========================================');
    console.log('SertiKu Certificate Registry HYBRID');
    console.log('Gas-Optimized Version (~79% savings)');
    ...
    console.log('Deploying CertificateRegistryHybrid contract...');
    ...
    // Wait for deployment with 120 second timeout
    ...
    try {
        ...
        if (envContent.includes('POLYGON_CONTRACT_ADDRESS=')) {
            ...
            envContent += `\n# Certificate Registry Hybrid Smart Contract (Gas Optimized)\nPOLYGON_CONTRACT_ADDRESS=${contractAddress}\n`;
        }
    ...
    const abiPath = path.join(__dirname, 'CertificateRegistryHybrid.abi.json');
    fs.writeFileSync(abiPath, JSON.stringify(abi, null, 2));
    console.log('✓ ABI saved to scripts/CertificateRegistryHybrid.abi.json');

    console.log('\n==========================================');
    console.log('GAS COMPARISON:');
    console.log('Old Contract: ~721,000 gas per certificate');
    console.log('New Contract: ~150,000 gas per certificate');
    console.log('Savings: ~79% per transaction');
    console.log('==========================================');
