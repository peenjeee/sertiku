            ...
            <form action="{{ route('lembaga.template.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1">
                    <div class="relative">
                        <input type="text" name="search" value="{{ request('search') }}"
                            ...
                            class="w-full pl-10 pr-4 py-2 bg-white/50 border border-white/10 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm placeholder-gray-500 text-gray-800">
                ...
                <div class="w-full sm:w-48">
                    <select name="status" onchange="this.form.submit()"
                        class="w-full px-4 py-2 bg-white/50 border border-white/10 rounded-lg focus:ring-blue-500 focus:border-blue-500 text-sm text-gray-800">
                        <option value="">Semua Status</option>
                        <option value="active" {{ request('status') == 'active' ? 'selected' : '' }}>Aktif</option>
                        <option value="inactive" {{ request('status') == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                </div>
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition shadow-lg shadow-blue-500/30">
                    Cari
                </button>
            </form>
        </div>
                    ...
                    <div class="glass-card rounded-2xl overflow-hidden group hover:shadow-lg transition">
                        ...
                        <div
                            ...
                            <div
                                ...
                                <a href="{{ route('lembaga.template.editPosition', $template) }}"
                                    class="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white hover:bg-blue-600 transition"
                                    title="Edit Posisi Nama">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
