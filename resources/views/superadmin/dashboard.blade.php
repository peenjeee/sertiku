...
<x-layouts.superadmin title="Dashboard – Super Admin SertiKu">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="glass-card rounded-2xl p-6 animate-fade-in-up">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('superadmin.admins') }}" class="text-purple-400 text-sm hover:text-purple-300">
                    Kelola →
                </a>
            </div>
            <div class="space-y-3">
                @forelse($admins as $admin)
                    <div class="flex items-center gap-3 p-3 rounded-xl bg-white/5">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center overflow-hidden
                            ...
                            @else
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($admin->name) }}&email={{ urlencode($admin->email) }}&background={{ $admin->is_superadmin ? 'A855F7' : '3B82F6' }}&color=fff&bold=true&size=40"
                        ...
                        @if($admin->is_superadmin)
                            <span class="px-2 py-1 rounded-full bg-purple-500/20 text-purple-400 text-xs">Super Admin</span>
                        @else
        ...
        <div class="glass-card rounded-2xl p-6 animate-fade-in-up">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-white">User Terbaru</h2>
            </div>
            <div class="space-y-3">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="glass-card rounded-xl p-4 text-center animate-fade-in-up">
            <p class="text-2xl font-bold text-purple-400">{{ $stats['total_superadmins'] }}</p>
            <p class="text-white/50 text-sm">Super Admins</p>
        </div>
        <div class="glass-card rounded-xl p-4 text-center animate-fade-in-up">
