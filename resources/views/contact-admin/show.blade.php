...
<x-dynamic-component :component="$layoutComponent" :title="'Tiket #' . $ticket->id . ' – SertiKu'">
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6 animate-fade-in-up">
            <div>
                <a href="{{ route('contact.admin') }}" class="text-white/50 hover:text-white text-sm flex items-center gap-1 mb-2 transition">
        ...
        {{-- Chat Messages --}}
        <div class="rounded-2xl overflow-hidden animate-fade-in-up mb-6" style="background: linear-gradient(180deg, #0F172A 0%, #1E293B 100%); border: 1px solid rgba(255,255,255,0.1);">
            ...
            <div class="p-4 space-y-4 max-h-96 overflow-y-auto" id="chatContainer">
                ...
                <div class="flex items-start gap-3 {{ $message->is_from_admin ? 'flex-row-reverse' : '' }}">
                    ...
                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0
                        {{ $message->is_from_admin ? 'bg-gradient-to-br from-green-500 to-emerald-600' : 'bg-gradient-to-br from-blue-500 to-indigo-600' }}">
                        {{ strtoupper(substr($message->sender->name ?? ($message->is_from_admin ? 'A' : 'U'), 0, 1)) }}
                    </div>

                    {{-- Message Bubble --}}
                    <div class="max-w-[75%] {{ $message->is_from_admin ? 'text-right' : '' }}">
                        <div class="px-4 py-3 rounded-2xl {{ $message->is_from_admin ? 'bg-green-500/20 rounded-tr-none' : 'bg-white/10 rounded-tl-none' }}">
        ...
        <div class="rounded-2xl p-4 animate-fade-in-up" style="background: linear-gradient(180deg, #0F172A 0%, #1E293B 100%); border: 1px solid rgba(255,255,255,0.1);">
            <form action="{{ route('contact.admin.send', $ticket) }}" method="POST" class="flex gap-3">
                ...
                <button type="submit" class="px-6 py-3 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 text-white font-medium hover:opacity-90 transition flex items-center gap-2">
        ...
        @else
        <div class="rounded-2xl p-4 text-center animate-fade-in-up" style="background: linear-gradient(180deg, #0F172A 0%, #1E293B 100%); border: 1px solid rgba(255,255,255,0.1);">
            <p class="text-white/50 text-sm">Tiket ini sudah ditutup. Buat tiket baru jika ada pertanyaan lain.</p>
            <a href="{{ route('contact.admin') }}" class="inline-block mt-3 px-4 py-2 rounded-lg bg-white/10 text-white text-sm hover:bg-white/20 transition">
                Buat Tiket Baru
            </a>
        </div>
        @endif
