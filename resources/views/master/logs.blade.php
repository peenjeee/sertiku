    ...
    <div class="glass-card rounded-2xl p-4 mb-6 animate-fade-in-up">
        <form method="GET" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[200px]">
                <select name="action" class="w-full px-4 py-2 rounded-xl bg-white/10 border border-white/10 text-white text-sm focus:outline-none focus:border-purple-500">
            ...
            <button type="submit" class="px-6 py-2 rounded-xl bg-purple-500 text-white text-sm hover:bg-purple-600 transition">
                Filter
            </button>
            @if(request()->hasAny(['action', 'date', 'user_id']))
            <a href="{{ route('master.logs') }}" class="px-6 py-2 rounded-xl bg-white/10 text-white/70 text-sm hover:bg-white/20 transition">
                Reset
            </a>
            @endif
        </form>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="hover:bg-white/5 transition">
                        ...
                        <td class="py-4 px-5">
                            <span class="px-2 py-1 rounded-lg bg-{{ $log->color }}-500/20 text-{{ $log->color }}-400 text-xs">
