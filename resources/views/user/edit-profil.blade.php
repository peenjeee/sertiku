...
<x-layouts.user title="Edit Profil – SertiKu">
    ...
    {{-- Session Messages --}}
    @if(session('success'))
    <div class="mb-6 p-4 rounded-xl bg-green-500/20 border border-green-500/30 text-green-400 text-sm flex items-center gap-2 animate-fade-in-up">
    ...
    <div x-data="photoUpload()" class="glass-card rounded-2xl p-5 lg:p-6 mb-6 animate-fade-in-up">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-6">
            ...
            <div class="relative group">
                <div class="w-24 h-24 lg:w-28 lg:h-28 rounded-full overflow-hidden bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
                    @if($user->avatar)
                    <img :src="previewUrl || '{{ asset('storage/' . $user->avatar) }}'" alt="Avatar" class="w-full h-full object-cover">
                    @else
                    <template x-if="previewUrl">
                        <img :src="previewUrl" alt="Avatar" class="w-full h-full object-cover">
                    </template>
                    <template x-if="!previewUrl">
                        <span class="text-white font-bold text-3xl lg:text-4xl">{{ strtoupper(substr($user->name ?? 'U', 0, 1)) }}</span>
                    </template>
                    @endif
                </div>
        ...
        <div x-show="showModal"
            ...
            <div @click.away="showModal = false"
                ...
                <div class="flex items-center justify-between p-5 border-b border-white/10">
                    ...
                    <button @click="showModal = false" class="p-1 rounded-lg hover:bg-white/10 transition">
                ...
                <form action="{{ route('user.profil.avatar') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-5">
                        ...
                        <div @dragover.prevent="isDragging = true"
                             ...
                             class="relative p-8 border-2 border-dashed rounded-xl text-center transition cursor-pointer hover:border-blue-500/50"
                            ...
                            <template x-if="previewUrl">
                                <div>
                                    <img :src="previewUrl" alt="Preview" class="w-32 h-32 mx-auto rounded-full object-cover border-4 border-blue-500/30">
                        ...
                        {{-- Error Message --}}
                        <template x-if="error">
                            <div class="mt-4 p-3 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 text-sm" x-text="error"></div>
                        </template>
                    </div>
