<x-layouts.app title="Pengaturan Akun - SertiKu">
    <main class="min-h-screen bg-gradient-to-b from-[#0F172A] via-[#1E293B] to-[#0F172A] px-4 py-10">
        <div class="mx-auto max-w-3xl">
            ...
            <div class="space-y-6">
                ...
                <div class="rounded-2xl bg-white/5 border border-white/10 p-6">
                    ...
                    <form action="{{ route('settings.profile') }}" method="POST" class="space-y-4">
                        ...
                        {{-- Avatar Upload --}}
                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6 pb-6 border-b border-white/10">
                            <div class="relative group">
                                <img id="avatar-preview" 
                                     src="{{ $user->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=3B82F6&color=fff' }}" 
                                     alt="Profile" 
                                     class="w-20 h-20 rounded-full border-2 border-[#3B82F6] object-cover">
                                <label for="avatar-input" class="absolute inset-0 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition cursor-pointer">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    {{-- Upload Form --}}
                                    <form id="avatar-form" action="{{ route('settings.avatar') }}" method="POST" enctype="multipart/form-data" class="inline">
                                        @csrf
                                        ...
                                        <button type="submit" id="upload-btn" class="hidden px-3 py-1.5 bg-emerald-500 text-white text-xs font-medium rounded-lg hover:bg-emerald-600 transition">
                                            ✓ Upload
                                        </button>
                                    </form>
                                    ...
                                    {{-- Remove Avatar --}}
                                    @if($user->avatar)
                                    <form action="{{ route('settings.avatar.remove') }}" method="POST" class="inline" onsubmit="return confirm('Hapus foto profil?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-3 py-1.5 bg-red-500/20 border border-red-500/30 text-red-400 text-xs font-medium rounded-lg hover:bg-red-500/30 transition">
                                            🗑️ Hapus
                                        </button>
                                    </form>
                                    @endif
                        ...
                        <div class="pt-4">
                            <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-[#1E3A8F] to-[#3B82F6] text-white text-sm font-medium rounded-lg hover:brightness-110 transition">
                                Simpan Perubahan
                            </button>
                        </div>
                    </form>
                ...
                <div class="rounded-2xl bg-white/5 border border-white/10 p-6">
                    ...
                    <form action="{{ route('settings.password') }}" method="POST" class="space-y-4">
                        ...
                        <div class="pt-4">
                            <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-sm font-medium rounded-lg hover:brightness-110 transition">
                                Ubah Password
                            </button>
                        </div>
                    </form>
                ...
                <div class="rounded-2xl bg-red-500/10 border border-red-500/30 p-6">
                    ...
                    <p class="text-sm text-red-300/80 mb-4">
                        Menghapus akun akan menghapus semua data Anda secara permanen termasuk sertifikat yang telah diterbitkan.
                    </p>

                    <button type="button" onclick="openDeleteModal()" class="px-4 py-2 bg-red-500/20 border border-red-500/50 text-red-400 text-sm font-medium rounded-lg hover:bg-red-500/30 transition">
                        Hapus Akun Saya
                    </button>
                </div>
            </div>
            ...
            {{-- Back Link --}}
            <div class="mt-8">
                <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 text-[#BEDBFF]/70 hover:text-white text-sm transition">
        ...
        <div class="bg-[#1E293B] rounded-2xl p-6 max-w-md w-full border border-white/10">
            ...
            <form action="{{ route('settings.delete') }}" method="POST">
                ...
                <div class="space-y-2 mb-4">
                    <label class="text-sm text-white">Ketik <strong class="text-red-400">HAPUS</strong> untuk konfirmasi</label>
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="closeDeleteModal()" class="flex-1 px-4 py-2.5 bg-white/10 text-white text-sm font-medium rounded-lg hover:bg-white/20 transition">
                        Batal
                    </button>
                    <button type="submit" class="flex-1 px-4 py-2.5 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition">
                        Hapus Akun
                    </button>
                </div>
            </form>
