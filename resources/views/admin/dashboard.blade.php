<x-layouts.admin>
        ...
        <div class="stat-card-green rounded-2xl p-5 hover-lift animate-fade-in-up stagger-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-500 text-xs mt-1">{{ $stats['sertifikat_aktif'] > 0 ? 'Aktif dan valid' : 'Belum ada data' }}</p>
                </div>
                <div class="w-12 h-12 icon-circle-green rounded-xl flex items-center justify-center">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        ...
        <div class="stat-card-red rounded-2xl p-5 hover-lift animate-fade-in-up stagger-5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-500 text-xs mt-1">{{ $stats['sertifikat_dicabut'] > 0 ? 'Dicabut' : 'Belum ada data' }}</p>
                </div>
        ...
        <div class="stat-card-orange rounded-2xl p-5 hover-lift animate-fade-in-up stagger-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-500 text-xs mt-1">{{ $stats['total_verifikasi'] > 0 ? 'Permintaan verifikasi' : 'Belum ada data' }}</p>
                </div>
    ...
    <div class="glass-card rounded-2xl p-6 mb-8 animate-fade-in">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('admin.analytics') }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">Lihat Semua</a>
        </div>
            ...
            <table class="w-full">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-50 hover:bg-gray-50 transition">
                        <td class="py-3 px-2">
                            <span class="text-gray-800 text-sm font-mono">{{ Str::limit($cert->certificate_number, 18) }}</span>
                        </td>
                        <td class="py-3 px-2">
                            <span class="text-gray-700 text-sm">{{ $cert->recipient_name }}</span>
                        </td>
                        <td class="py-3 px-2">
                            <span class="text-gray-600 text-sm">{{ $cert->user->institution_name ?? $cert->user->name }}</span>
                        </td>
                        <td class="py-3 px-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 animate-fade-in">
        <!-- Kelola Pengguna -->
        <a href="{{ route('admin.users') }}" class="glass-card rounded-2xl p-5 hover-lift flex items-center gap-4 group">
            <div class="w-12 h-12 icon-circle-blue rounded-xl flex items-center justify-center">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
                </svg>
        ...
        <!-- Analytics -->
        <a href="{{ route('admin.analytics') }}" class="glass-card rounded-2xl p-5 hover-lift flex items-center gap-4 group">
            <div class="w-12 h-12 icon-circle-purple rounded-xl flex items-center justify-center">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Backup & Export -->
        <a href="{{ route('admin.backup') }}" class="glass-card rounded-2xl p-5 hover-lift flex items-center gap-4 group">
            <div class="w-12 h-12 icon-circle-orange rounded-xl flex items-center justify-center">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
